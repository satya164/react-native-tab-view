...
export default class TabView<T: *> extends React.Component<Props<T>, State> {
  ...
  constructor(props: Props<T>) {
    ...
    const { navigationState, explicitPanX, explicitOffsetX } = this.props;
    const layout = {
      ...this.props.initialLayout,
      measured: false,
    };

    const panX =  explicitPanX || new Animated.Value(0);
    const offsetX = explicitOffsetX || new Animated.Value(-navigationState.index * layout.width);
